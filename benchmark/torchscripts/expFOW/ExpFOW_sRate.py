...
class FOW(nn.Module):
    ...
    def forward(self, x):
        ...
        self.lastAi=eA
        return eMu,eTau,eLambda,eA,self.pMu
    ...
    def lossUnderNormal(self, x_recon, x, mu, logvar):
        ...
        ELBO=torch.sigmoid(logLikelyHood+logPMu+logPTau+logPA+logPLambda-(logEMu+logETau+logEA+logELambda))
...
def pretrainModel(device, prefixTag, saveTag):
    ...
    # model = FOW(input_size, hidden_size, latent_size).to(device)
    model = FOW(input_dim, hidden_dim, latent_dim, num_layers, num_heads)
...
def main():
    ...
    pretrainModel(device, prefixTag, "linearFOW_sRate.pt")
