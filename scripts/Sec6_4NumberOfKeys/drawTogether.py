...
def runPeriod(exePath, period, resultPath, configTemplate="config.csv"):
    ...
    editConfig(configTemplate, exePath + configFname, "keyRange", period)
    # prepare new file
    # run
...
def main():
    ...
    commonBasePath = os.path.abspath(os.path.join(os.getcwd(), "../..")) + "/results/Sec6_4NumberOfKeys/"

    figPath = os.path.abspath(os.path.join(os.getcwd(), "../..")) + "/figures/"
    configTemplate = exeSpace + "config.csv"
    #periodVec = [7, 8, 9, 10, 11, 12]
    periodVec = [10,20,50,100,200,500,1000,2000,5000]
    ...
    groupLine.DrawFigureYnormal(periodAll, np.array(errAll)*100.0, methodTags,"#Key","Error (%)" , 0, 1, figPath + "sec6_4_NumberKey_err",
                                True)
    groupLine.DrawFigureYnormal(periodAll, np.array(lat95All), methodTags,"#Key","95% latency (ms)", 0, 1, figPath + "sec6_4_NumberKey_lat",True)
