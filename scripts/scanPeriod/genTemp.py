...
def writeTemplateCsv(lat,eng):
    ...
    for i in range(len(lat)):
        if (lat[i]>ts):
            ts=lat[i]
            idxMaxLat=i
    CStream_eng=eng[idxMinEng]
    CStream_lat=lat[idxMinEng]
    ...
    with open("plans_temp.csv","w") as csvfile: 
        ...
        writer.writerow(["CStream",CStream_eng,CStream_lat,0,0])
        ...
        writer.writerow(["NS",CStream_eng,CStream_lat,0,0])
def writeTemplate2ndCsv(lat,eng):
    ...
    CStream_eng=eng[idxMinEng2]
    CStream_lat=lat[idxMinEng2]
    #print(lat2Idex,CStream_eng,CStream_lat)
