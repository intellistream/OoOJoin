...
def runPeriod(exePath, period, resultPath, configTemplate="config.csv"):
    ...
    editConfig(configTemplate, exePath + configFname, "maxArrivalSkewMs", period)
    # prepare new file
    # run
...
def main():
    ...
    commonBasePath = os.path.abspath(os.path.join(os.getcwd(), "../..")) + "/results/Sec6_3ModelingStockSynth/"

    figPath = os.path.abspath(os.path.join(os.getcwd(), "../..")) + "/figures/"
    configTemplate = exeSpace + "config.csv"
    periodVec = [90,100,110,120,150,200,300,400,500]
    #periodVec = [7, 10, 12]
    ...
    groupLine.DrawFigureYnormal(periodAll, np.array(errAll)*100.0, methodTags,"max"+r"$(t_a-t_e)$ (ms)","Error (%)" , 0, 1, figPath + "sec6_3_modeling_stock_synth",
                                True)
    print(errAll)
